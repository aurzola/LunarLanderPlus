...
// Replicates clk_ll_apll_set_config(false,...) without the APLL calibration
// that rtc_clk_apll_coeff_set() performs. Calibration waits forever on this
// board (OR_CAL_END never sets), so configure the SDM directly and enable.
static void apll_set_config_nocal(uint32_t o_div, uint32_t sdm0, uint32_t sdm1, uint32_t sdm2)
{
...
static void set_dac_frequency(void)
{
    ...
    switch(g_video_signal.dac_frequency)
    {
        ...
        case DAC_FREQ_PAL_NTSC_13_5MHz: //=13500001
            ...
            apll_set_config_nocal(7, 205, 76, 20); rtc_clk_apll_enable(true);
            break;

        case DAC_FREQ_PAL_NTSC_6_75MHz: // =6.750000
            ESP_LOGI(TAG, "DAC clock configured to 6.75 MHz. BT.601 PAL/NTSC 320 pixels.");
            apll_set_config_nocal(16, 205, 76, 20); rtc_clk_apll_enable(true);
            break;
...
static void setup_video_dac(void)
{
    ...
    DBG("VIDEO[dac] tx_start ok\n");

    ESP_ERROR_CHECK(esp_intr_enable(i2s_interrupt_handle));
    DBG("VIDEO[dac] intr_enable ok\n");
...
void video_init(uint16_t width, uint16_t height, FRAME_BUFFER_FORMAT fb_format, VIDEO_MODE mode, bool hires_pixel)
{
	...
	setup_video_dac();
    DBG("VIDEO[init] setup_video_dac returned\n");
