...
- **Demo / attract mode (8/8/2026; niveles al azar 1..12 desde 13/8/2026; al azar en CADA ciclo
  ...
  **autopilot** (`Game::runDemoAI()`) hacia `demoTargetX/Y`. Cada ciclo re-tira el nivel al azar
  (cualquier luna/efecto; `DEMO_LEVEL_FIRST=0` — antes abría siempre en Luna nivel 1 con la
  cisterna para el showcase de repostaje, revertido). La cisterna sigue forzada en la demo
  (`force=true`) y el combustible arranca al 30 % (`FUEL_MAX·0.3`), así el repostaje aéreo puede
  aparecer en cualquier luna donde el tanque esté permitido (excluido de Ganímedes por los
  anillos y apagado por `isolateForWormhole`). `DEMO_FORCE_TANKER_CRASH=0` (el showcase de choque
