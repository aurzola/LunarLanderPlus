...
- **Pantalla de título (7/8/2026)** (`STATE_WAITING`): título **`LUNAR LANDER++`** en **negrita**
  ...
  **patas rediseñadas (10/8/2026)**: V-struts con doble miembro paralelo + zigzag de
  cross-bracing y footpad rectangular, tanto en las patas izquierda/derecha como en la central),
...
| `src/renderer_vga.h/cpp` | `RendererVGA : RendererCanvas`. Doble buffer: `game.draw()` pinta en `fbBack` (estático 76.8 KB); `flush()` aplica **dithering Bayer 8×8** in-place sobre `fbBack` (precomputado, umbrales 2–254; valores 0/255 pasan sin cambios) y luego hace `waitVBlank()` + `memcpy` a `fbFront`. `pixel()` escribe brillo `WHITE=200` (en vez de 255) para que el dithering añada textura incluso en líneas sólidas. El dithering se ejecuta fuera del blanking (el `memcpy` dentro de vblank es rápido) — sin tearing. **No afecta al CRT** (código exclusivo de VGA) |
