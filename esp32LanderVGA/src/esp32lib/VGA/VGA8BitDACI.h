...
class VGA8BitDACI : public VGAI2SDynamic< BLpx1sz16sw1sh0, GraphicsW8 > // GraphicsW8 (=) Graphics<ColorW8, BLpx1sz8sw0sh0, CTBIdentity>
{
  public:
	...
	// --- Lunar Lander fork: external frame store instead of the 300 KB
	// default framebuffer (no PSRAM on the WROOM-32). The store is a
	// 320x240 8-bit grey canvas owned by the sketch; the driver maps its
	// per-line row pointers into it and scales 2x in the line ISR.
	void setFrameStore(uint8_t *store, int storeW, int storeH)
	{
		frameStore_ = store;
		frameStoreW_ = storeW;
		frameStoreH_ = storeH;
	}
	uint8_t **allocateFrameBuffer() override;
	// Block until the vertical blanking window of the current frame, where a
	// memcpy from the back buffer to the frame store is safe (the line ISR is
	// only producing sync/blank lines at that point).
	void waitVSync()
	{
		vSyncPassed = false;
		while (!vSyncPassed) delay(0);
	}
