...
#include "src/game.h"
#include "src/renderer_vga.h"
#include "src/audio.h"
#include "src/nunchuck.h"
#include "src/esp32lib/VGA/VGA8BitDACI.h"
...
// VGA output
const int PIN_DAC = 25;   // GPIO25 = DAC1 -> R/G/B (divisor de voltaje)
...
static Game game;
static VGA8BitDACI display;
static RendererVGA *renderer = NULL;

// Double buffer VGA: game.draw() paints into fbBack; RendererVGA::flush()
// waits for the vertical blanking window and memcpy's fbBack -> fbFront. The
// VGA line ISR only ever reads fbFront, so the DMA never scans a half-updated
// frame (mismo patrón de doble buffer del compuesto).
// fbFront is heap-allocated in setup() (mismo patrón que el FB de video del
// compuesto): el segmento de DRAM estático no alcanza para dos buffers de
// 76.8 KB, pero el heap sí tiene los dos bloques contiguos si se pide al
// arrancar, antes de Audio::begin() y de display.init().
static uint8_t fbBack[XRES * YRES];
static uint8_t *fbFront = NULL;
...
static void vgaWaitVBlank()
{
    display.waitVSync();
}

#if VGA_TEST_PATTERN
// Rampa de grises + rejilla para validar en el monitor: 255 niveles de gris,
// escalado 2x (el canvas 320x240 debe llenar la pantalla 640x480) y sync HS/VS.
static void drawTestPattern(Renderer &r)
{
    r.clear();

    // Escalera de luminancia: 5 bandas a lo ancho, arriba.
    const int steps[] = {0, 64, 128, 192, 255};
    for (int i = 0; i < 5; i++) {
        for (int y = 10 + i * 20; y < 10 + (i + 1) * 20; y++)
            for (int x = 0; x < XRES; x++)
                r.pixelShade((float)x, (float)y, steps[i]);
    }

    // Rejilla (verticales cada 20 px, horizontales cada 20 px).
    for (int x = 0; x < XRES; x += 20)
        for (int y = 118; y < YRES; y++)
            r.pixelShade((float)x, (float)y, 80);
    for (int y = 118; y < YRES; y += 20)
        for (int x = 0; x < XRES; x++)
            r.pixelShade((float)x, (float)y, 80);

    // Rampa continua 0..255.
    for (int x = 0; x < XRES; x++) {
        int b = x * 255 / (XRES - 1);
        for (int y = 128; y < 208; y++)
            r.pixelShade((float)x, (float)y, b);
    }

    // Marco blanco: comprueba overscan/centrado y que el escalado llena la pantalla.
    r.line(0, 0, XRES - 1, 0);
    r.line(0, 0, 0, YRES - 1);
    r.line(XRES - 1, 0, XRES - 1, YRES - 1);
    r.line(0, YRES - 1, XRES - 1, YRES - 1);

    r.text((XRES - (int)strlen("VGA TEST 640x480") * 6) / 2.0f, 220,
           "VGA TEST 640x480");
...
void setup()
{
    ...
    // Grab the 76.8 KB VGA front buffer first so the heap hands it out as one
    // contiguous block (Audio::begin() and the driver DMA come afterwards).
    fbFront = (uint8_t *)malloc((size_t)XRES * (size_t)YRES);
