...
- **TEMP**: `RING_FOG_BRIGHT=0` desactiva la niebla de las bandas de Ganímedes (decisión de diseño). `DEMO_LEVEL_FORCE=0` en PC y sketch (demo elige nivel al azar 1..12). **`START_LEVEL=2` (TEMP 14/9/2026: primer nivel = Ío/terremoto; REVERTIR a 1 = LUNA)**. **`DEMO_QUAKE_FIRST=true` (TEMP 14/9/2026: la demo SIEMPRE abre su nivel en Ío/terremoto; REVERTIR a false = showcase de wormhole + niveles al azar)**. **`DEMO_SPAWN_Y_MIN/MAX` (100/260)**: la nave de la demo aparece siempre a una altitud aleatoria en esa banda. `FOG_SCREEN_TOP=68` (cuadro de limpieza del HUD a la altura de MEM).
