...
- HUD: `L<n> SCORE`/`FUEL`/`ANG`/`PWR` en `(22,22)`/`(22,32)`/`(22,42)`/`(22,52)`;
  ...
  **`SCL`/`VWS`/`TK` de debug (TEMP)**: `SCL` (`ship.scale`), `VWS` (`viewScale`) y `TK`
  (`Tanker::drawScaleFor`) en `(250,92)`/`(250,102)`/`(250,112)` mientras la demo esté activa —
  herramienta para validar el escalado/zoom en CRT; quitar al cerrar la rama.
...
- **Demo / attract mode (8/8/2026; niveles al azar 1..12 desde 13/8/2026)**: tras
  ...
  **Ciclo fijo en Luna (25/8/2026, rama `tanker-docking`)**: la demo SIEMPRE abre en
  `DEMO_LEVEL_FIRST=1` (Luna), donde la cisterna es forzosa (`level==1`) y el combustible
  arranca al 30 % (`FUEL_MAX·0.3`), para que el attract muestre el **repostaje aéreo completo**:
  el autopilot vuela al drogue, dockea, reposta, y luego crucero-desciende hacia el pad
  (`demoHoldAltitude` ahora desciende, `desVY ∈ [−0.12, 0.08]`; `demoTargetY = terrain.yAt(pad)`).
  `DEMO_FORCE_TANKER_CRASH=0` (el showcase de choque quedó desactivado). Tras ese ciclo, cada
  nuevo ciclo re-tira al azar 1..12. **Sensores ignorados en demo**: `readInputs()` del sketch
...
- **TEMP**: `RING_FOG_BRIGHT=0` desactiva la niebla de las bandas de Ganímedes (decisión de diseño). `DEMO_LEVEL_FORCE=0` en PC y sketch (demo elige nivel al azar 1..12). **`DEMO_WORMHOLE_FIRST=false` (16/9/2026: la demo ya NO fuerza el showcase de wormhole; cada ciclo elige nivel al azar 1..12, cualquier luna/efecto)**. **`DEMO_SPAWN_Y_MIN/MAX` (100/260)**: la nave de la demo aparece siempre a una altitud aleatoria en esa banda. `FOG_SCREEN_TOP=68` (cuadro de limpieza del HUD a la altura de MEM). **`SCL`/`VWS`/`TK` en el HUD (TEMP, rama `tanker-docking`)**: debug de `ship.scale`, `viewScale` y `Tanker::drawScaleFor` en `(250,92)`/`(250,102)`/`(250,112)` mientras la demo está activa; quitar al cerrar la rama.
